        ...
        async def refresh_model_info():
            ...
            with model_info_row:
                ...
                if status and status.get("llm_provider"):
                    ...
                    display_text = f"{provider_type}-{profile_id}({model_id})"
                    if persona_name:
                        display_text += f"|{persona_name}"
                    with (
                        ui.card()
                        .classes("w-full")
                        ...
                        with ui.row().classes("items-center gap-3 px-4 py-2"):
                            ...
                            ui.label(f"当前模型：{display_text}").classes("text-indigo-700 text-sm font-medium")
                else:
                    with (
                        ui.card()
                        .classes("w-full")
                            ...
                            async with websockets.connect(WS_URL) as ws:
                                ...
                                while True:
                                    ...
                                    if t == "start":
                                        ...
                                        agnes_label.set_content(f"[错误] {err_msg}")
                                        append_debug(f"[error] {err_msg}")
                                        break
                                    else:
                                        append_debug(f"[unknown] {data}")
                        except Exception as e:
                            agnes_label.set_content(f"[连接失败] {e}\n\n请检查服务是否已启动，并已激活模型。")
