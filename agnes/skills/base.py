...
class SkillResult(BaseModel):
    ...
    @classmethod
    def ok(
        cls, data: Any = None, execution_time_ms: float = 0.0, metadata: dict[str, Any] | None = None
    ) -> "SkillResult":
        """创建成功结果"""
        return cls(success=True, data=data, execution_time_ms=execution_time_ms, metadata=metadata or {})

    @classmethod
    def error(cls, error_type: str, message: str, execution_time_ms: float = 0.0) -> "SkillResult":
        """创建错误结果"""
        return cls(success=False, error_type=error_type, error_message=message, execution_time_ms=execution_time_ms)
